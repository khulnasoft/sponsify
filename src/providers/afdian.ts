...
import type { Provider, SponsifyConfig, Sponsorship } from "../types";
...
export async function fetchAfdianSponsors(
  ...
  if (!userId || !token) throw new Error("Afdian id and token are required");
  ...
  do {
    ...
    page += 1;
    if (sponsorshipData?.ec !== 200) break;
    pages = sponsorshipData.data.total_page;
    if (!includePurchases) {
      sponsorshipData.data.list = sponsorshipData.data.list.filter(
        (sponsor: any) => {
          const current = sponsor.current_plan;
          if (!current || current.product_type === 0) return true;
          return false;
        },
      );
    }
    if (purchaseEffectivity > 0) {
      sponsorshipData.data.list = sponsorshipData.data.list.map(
        (sponsor: any) => {
          const current = sponsor.current_plan;
          if (!current || current.product_type === 0) return sponsor;
          const expireTime =
            current.update_time + purchaseEffectivity * 24 * 3600;
  ...
  const processed = sponsors.map((raw: any): Sponsorship => {
    ...
    if (name.startsWith("爱发电用户_")) name = raw.user.user_id.slice(0, 5);
    ...
    return {
      ...
      expireAt: expireTime
        ? new Date(expireTime * 1000).toISOString()
        : undefined,
