...
export async function fetchPatreonSponsors(
  ...
  do {
    ...
    sponsors.push(
      ...sponsorshipData.data
        ...
        .filter(
          (membership: any) => membership.attributes.patron_status !== null,
        )
        .map((membership: any) => ({
          membership,
  ...
  const processed = sponsors.map(
    (raw: any): Sponsorship => ({
      ...
      monthlyDollars: ["former_patron", "declined_patron"].includes(
        raw.membership.attributes.patron_status,
      )
        ? -1
        : Math.floor(
            raw.membership.attributes.currently_entitled_amount_cents / 100,
          ),
