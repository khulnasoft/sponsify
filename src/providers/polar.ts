...
export const PolarProvider: Provider = {
  ...
  fetchSponsors(config) {
    return fetchPolarSponsors(
      config.polar?.token || config.token!,
      config.polar?.organization,
    );
  },
};

export async function fetchPolarSponsors(
  token: string,
  organization?: string,
): Promise<Sponsorship[]> {
  if (!token) throw new Error("Polar token is required");
  if (!organization) throw new Error("Polar organization is required");
  ...
  if (!orgId) throw new Error(`Polar organization "${organization}" not found`);
