...
import type { ImageFormat, SponsifyConfig, Sponsorship } from "../types";

async function fetchImage(url: string) {
  const arrayBuffer = await $fetch(url, {
    responseType: "arrayBuffer",
    headers: {
      "User-Agent": `Mozilla/5.0 Chrome/124.0.0.0 Safari/537.36 Sponsify/${version}`,
    },
  });
...
export async function resolveAvatars(
  ships: Sponsorship[],
  getFallbackAvatar: SponsifyConfig["fallbackAvatar"],
  t = consola,
) {
  const fallbackAvatar = await (() => {
    ...
    if (getFallbackAvatar) return getFallbackAvatar;
  })();
  ...
  return Promise.all(
    ships.map((ship) =>
      limit(async () => {
        ...
        const pngBuffer = await fetchImage(ship.sponsor.avatarUrl).catch(
          (e) => {
            t.error(
              `Failed to fetch avatar for ${ship.sponsor.login || ship.sponsor.name} [${ship.sponsor.avatarUrl}]`,
            );
            t.error(e);
