...
export async function generateBadge(
  ...
  if (
    preset.name &&
    preset.name.maxLength &&
    name.length > preset.name.maxLength
  ) {
    if (name.includes(" ")) name = name.split(" ")[0];
    else name = `${name.slice(0, preset.name.maxLength - 3)}...`;
  }
...
export class SvgComposer {
  ...
  async addSponsorLine(sponsors: Sponsorship[], preset: BadgePreset) {
    ...
    const sponsorLine = await Promise.all(
      sponsors.map(async (s, i) => {
        const x = offsetX + preset.boxWidth * i;
        const y = this.height;
        const radius = s.sponsor.type === "Organization" ? 0.1 : 0.5;
        return await generateBadge(
          x,
          y,
          s.sponsor,
          preset,
...
<!-- Generated by https://github.com/khulnasoft/sponsorskit -->
