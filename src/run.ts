...
export async function run(inlineConfig?: SponsifyConfig, t = consola) {
  t.log(`\n${c.magenta(c.bold("SponsiFy"))} ${c.dim(`v${version}`)}\n`);

  const fullConfig = await loadConfig(inlineConfig);
  const config = fullConfig as Required<SponsifyMainConfig>;
  ...
  if (!fs.existsSync(cacheFile) || config.force) {
    ...
    for (const i of providers) {
      ...
      sponsors =
        (await config.onSponsorsFetched?.(sponsors, i.name)) || sponsors;
    ...
    // Custom hook
    allSponsors =
      (await config.onSponsorsAllFetched?.(allSponsors)) || allSponsors;
    ...
    // Merge sponsors
    {
      ...
      function pushGroup(group: Sponsorship[]) {
        const existingSets = new Set(
          group.map((s) => sponsorsMergeMap.get(s)).filter(notNullish),
        );
      ...
      function matchSponsor(sponsor: Sponsorship, matcher: SponsorMatcher) {
        ...
        if (matcher.login && sponsor.sponsor.login !== matcher.login)
          return false;
        if (matcher.name && sponsor.sponsor.name !== matcher.name) return false;
        if (matcher.type && sponsor.sponsor.type !== matcher.type) return false;
        return true;
      }
      ...
      if (config.sponsorsAutoMerge) {
        for (const ship of allSponsors) {
          if (!ship.sponsor.socialLogins) continue;
          for (const [provider, login] of Object.entries(
            ship.sponsor.socialLogins,
          )) {
            const matched = allSponsors.filter(
              (s) => s.sponsor.login === login && s.provider === provider,
            );
            if (matched) pushGroup([ship, ...matched]);
          }
        }
      }
      ...
      function mergeSponsors(main: Sponsorship, sponsors: Sponsorship[]) {
        ...
        main.monthlyDollars = all.every((s) => s.monthlyDollars === -1)
          ...
          : all
              ...
              .reduce((a, b) => a + b.monthlyDollars, 0);
      ...
      for (const group of groups) {
        ...
        const sorted = [...group].sort(
          (a, b) => allSponsors.indexOf(a) - allSponsors.indexOf(b),
        );

        t.info(
          `Merging ${sorted.map((i) => c.cyan(`@${i.sponsor.login}(${i.provider})`)).join(" + ")}`,
        );

        for (const s of sorted.slice(1)) removeSponsors.add(s);
  ...
  if (config.renders?.length) {
    ...
    await Promise.all(
      config.renders.map(async (renderOptions) => {
        const mergedOptions = {
          ...fullConfig,
          ...renderOptions,
        };
        const renderer = builtinRenderers[mergedOptions.renderer || "tiers"];
        await applyRenderer(renderer, config, mergedOptions, allSponsors, t);
      }),
    );
  } else {
    const renderer = builtinRenderers[fullConfig.renderer || "tiers"];
    await applyRenderer(renderer, config, fullConfig, allSponsors, t);
  }
}

export async function applyRenderer(
  renderer: SponsifyRenderer,
  config: Required<SponsifyMainConfig>,
  renderOptions: Required<SponsifyRenderOptions>,
  ...
  if (renderOptions.filter)
    sponsors = sponsors.filter(
      (s) => renderOptions.filter(s, sponsors) !== false,
    );
  if (!renderOptions.includePrivate)
    sponsors = sponsors.filter((s) => s.privacyLevel !== "PRIVATE");

  if (!renderOptions.imageFormat) renderOptions.imageFormat = "webp";
...
function normalizeReplacements(replaces: SponsifyMainConfig["replaceLinks"]) {
  const array = (Array.isArray(replaces) ? replaces : [replaces]).filter(
    notNullish,
  );
  const entries = array
    .map((i) => {
