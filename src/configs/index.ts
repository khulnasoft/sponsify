...
export async function loadConfig(
  inlineConfig: SponsifyConfig = {},
): Promise<Required<SponsifyConfig>> {
  const env = loadEnv();
  ...
  const hasNegativeTier = !!config.tiers?.find(
    (tier) => tier && tier.monthlyDollars! <= 0,
  );
...
export function partitionTiers(
  ...
  sponsors
    ...
    .forEach((sponsor) => {
      const tier =
        tierMappings.find((t) => sponsor.monthlyDollars >= t.monthlyDollars) ??
        tierMappings[0];
