...
export async function run(fn: Runner, args: string[], options: DetectOptions = {}) {
  ...
  if (args.length === 1 && (args[0]?.toLowerCase() === '-v' || args[0] === '--version')) {
    ...
    console.log(`@khulnasoft/nx  ${c.cyan(`v${version}`)}`)
  ...
  if (args.length === 1 && ['-h', '--help'].includes(args[0])) {
    const dash = c.dim('-')
    console.log(c.green(c.bold('@khulnasoft/nx')) + c.dim(` use the right package manager v${version}\n`))
    ...
    console.log(c.yellow('\ncheck https://github.com/khulnasoft/nx for more documentation.'))
    return
  }
