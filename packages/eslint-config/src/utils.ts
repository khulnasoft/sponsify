...
const isCwdInScope = isPackageExists("@khulnasoft/eslint-config");
 ...
 * @example
 * ```ts
 * import { renameRules } from '@khulnasoft/eslint-config'
 *
 * export default [{
 *   rules: renameRules(
 *     {
 ...
 * @example
 * ```ts
 * import { renamePluginInConfigs } from '@khulnasoft/eslint-config'
...
export async function ensurePackages(
  ...
  if (result)
    await import("@khulnasoft/get-pkg").then((i) =>
