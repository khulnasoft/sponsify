...
export const CLI_TEMP_DIR = join(os.tmpdir(), "khulnasoft-ni");
  ...
  if (temp) {
    fs.writeFile(temp.path, data)
      .then(() => {
        ...
        if (!existsSync(directory)) fs.mkdir(directory, { recursive: true });
...
export function formatPackageWithUrl(pkg: string, url?: string, limits = 80) {
  return url
    ? terminalLink(pkg, url, {
        fallback: (_, url) =>
          pkg.length + url.length > limits ? pkg : pkg + c.dim(` - ${url}`),
      })
    : pkg;
}
