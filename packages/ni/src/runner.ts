...
export async function getCliCommand(
  ...
  if (isGlobal) return await fn(await getGlobalAgent(), args);

  let agent =
    (await detect({ ...options, cwd })) ||
...
export async function run(
  ...
  if (
    ...
    console.log(`@khulnasoft/ni  ${c.cyan(`v${version}`)}`);
  ...
  if (args.length === 1 && ["-h", "--help"].includes(args[0])) {
    ...
    console.log(
      c.yellow(
        "\ncheck https://github.com/khulnasoft/ni for more documentation.",
      ),
    );
    return;
  }
